...
class Picture extends Init
{
	...
	function lists(){
		$category_id = input('param.category_id');
		$pictures = $this->model->get_list(['category_id'=>$category_id],'is_top desc,id desc',9,1);
		$recommend_list = $this->model->get_list(['is_recommend'=>'1'],'is_top desc,id desc',8); //推荐文章
		$hot_list = $this->model->get_list([],['hits desc'],8); //热门文章
		$breadcrumb = $this->category_model->breadcrumb($category_id);
		$second_categorys = $this->category_model->get_second_categorys($category_id);
		$this->seo['title'] = $this->categorys[$category_id]['name'].'-'.$this->seo['title'];
		if($this->categorys[$category_id]['meta_keywords']){ $this->seo['keywords'] = $this->categorys[$category_id]['meta_keywords'].','.$this->seo['keywords'];}
		...
		return view($template,['lists'=>$pictures,'recommend_list'=>$recommend_list,'hot_list'=>$hot_list,'page'=>$pictures->render(),'breadcrumb'=>$breadcrumb,'second_categorys'=>$second_categorys,'seo'=>$this->seo]);
	}
