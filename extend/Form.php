...
class Form {
     ...
     * @return string
     */
    static public function ueditor($name, $value='',$label, $width ='100%', $height = '500',$toolbar = FALSE,$id='ueditor') {
      
        $string = '<div class="layui-form-item"><label class="layui-form-label">'.$label.'</label>';
        $string .= '<div class="layui-input-block"><script type="text/plain" id="'.$id.'" style="width:'.$width.';height:'.$height.';">'.$value.'</script></div>';
        $string .= '</div>';
        $string .= '<link rel="stylesheet" href="/static/ueditor/themes/default/css/ueditor.css">';
        $string .= '<script type="text/javascript" src="/static/ueditor/third-party/jquery.min.js"></script>';
        $string .= '<script type="text/javascript" src="/static/ueditor/ueditor.config.js"></script>';
        $string .= '<script type="text/javascript" src="/static/ueditor/ueditor.min.js"></script>';
        $string .= '<script type="text/javascript" src="/static/ueditor/lang/zh-cn/zh-cn.js"></script>';
        $width = (!empty($width)) ? $width : '90%';
        $height = (!empty($height)) ? $height : '500';
        $string .= '<script type="text/javascript">';

        $string .= 'var ue_'.$id.' = UE.getEditor(\''.$id.'\', {
            textarea : \''.$name.'\'
